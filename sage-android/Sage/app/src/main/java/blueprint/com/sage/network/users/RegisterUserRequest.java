...
/**
 * Created by kelseylam on 4/13/16.
 */
public class RegisterUserRequest extends BaseRequest {
    public RegisterUserRequest(final Activity activity, User user,
                             final Response.Listener<Session> onSuccess,
                             final Response.Listener<APIError> onFailure) {
        super(Method.POST, makeUrl(activity, null, "users", String.valueOf(user.getId()), "register"), convertToParams(user, "user", activity),
